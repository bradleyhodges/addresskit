...
Then(
    ...
    async function (dataTable) {
        ...
        for (const key in headers) {
            expect(this.current.response.headers.get(key)).to.equal(
                headers[key],
            );
        }
    },
...
Given("an empty address database", async () => {
    delete global.gnafLoaded;
    return clearAddresses();
});

Given("an address database with:", async (documentString) => {
    delete global.gnafLoaded;
...
Given("an address database is not loaded from gnaf", async () => {
...
Then(
    ...
    async function (status, documentString) {
        ...
        expect(await this.current.body()).to.deep.equal(
...
Then(
    ...
    async function (headerName, headerValue) {
        if (this.current.headers) {
            expect(this.current.headers[headerName.toString()]).to.not.be
                .undefined;
            expect(this.current.headers[headerName.toString()]).to.equal(
                headerValue,
            );
        } else {
            console.log({ current: this.current.response.headers });
            console.log({ body: await this.current.body() });
            expect(this.current.response.headers.get(headerName.toString())).to
                .not.be.undefined;
            expect(
                this.current.response.headers.get(headerName.toString()),
